...
const MIGRATION_NAME = '0004_category_percent';
  ...
  await transaction(async (conn) => {
    ...
    await conn.query(`
      ALTER TABLE categories
        ADD COLUMN percent DECIMAL(5,2) NULL AFTER icon;
    `);
...
run().then(() => {
  console.log('Category percent migration applied');
  process.exit(0);
}).catch((e) => {
  console.error('Category percent migration failed', e);
  process.exit(1);
});
