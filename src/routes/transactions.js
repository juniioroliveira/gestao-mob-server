...
router.get('/transactions', async (req, res, next) => {
  try {
    ...
    const sql = `SELECT id, user_id, account_id, category_id, member_id, type, amount, occurred_at, description, inscricao_federal, metadata, created_at
                 FROM transactions
...
router.post('/transactions', async (req, res, next) => {
  try {
    const { user_id, account_id, category_id, member_id, type, amount, occurred_at, description, inscricao_federal, metadata } = req.body || {};
    const uid = user_id || req.auth?.userId;
    if (!uid || !type || amount == null || !occurred_at) return res.status(400).json({ error: 'invalid_body' });
    const result = await query(
      'INSERT INTO transactions (user_id, account_id, category_id, member_id, type, amount, occurred_at, description, inscricao_federal, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [uid, account_id || null, category_id || null, member_id ?? null, type, amount, occurred_at, description || null, (inscricao_federal ?? null), metadata ? JSON.stringify(metadata) : null]
    );
    const [row] = await query('SELECT id, user_id, account_id, category_id, member_id, type, amount, occurred_at, description, inscricao_federal, metadata, created_at FROM transactions WHERE id = ?', [result.insertId]);
...
router.get('/transactions/pending-occurrence', async (req, res, next) => {
  try {
    const userId = Number(req.auth?.userId || req.query.userId);
    if (!userId) return res.status(400).json({ error: 'userId_required' });
    const limit = Math.min(Number(req.query.limit || 50), 200);
...
router.put('/transactions/:id/occurred', async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const occurred_at = req.body?.occurred_at;
    if (!id || !occurred_at) return res.status(400).json({ error: 'invalid_body' });
    await query('UPDATE transactions SET occurred_at = ? WHERE id = ?', [occurred_at, id]);
    const [row] = await query(
      'SELECT id, user_id, account_id, category_id, member_id, type, amount, occurred_at, description, inscricao_federal, metadata, created_at FROM transactions WHERE id = ?',
      [id]
    );
    if (!row) return res.status(404).json({ error: 'not_found' });
    res.json(row);
